...
*Note: This parameter is not implemented yet on the version 1.0.6.
...
On the 1.0.6, all search results for a request are returned in one time. In the future, as an optional behaviour, each result can be returned as separated messages progressively.
...
*Note: On the version 1.0.6, too many records can be returned larger than the specified `maxNSubRecords`, if the dataset has multiple volumes. This is a known problem and to be fixed in a future version.
