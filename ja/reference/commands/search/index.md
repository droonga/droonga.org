...
※註：このパラメータはバージョン 0.7 では未実装です。指定しても機能しません。
...
バージョン 0.7 ではすべての検索クエリの結果を一度にレスポンスとして返却する動作のみ対応していますが、将来的には、それぞれの検索クエリの結果を分割して受け取る（結果が出た物からバラバラに受け取る）動作にも対応する予定です。
